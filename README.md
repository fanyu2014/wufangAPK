...
12月10日教育加_3.0.7[apk]：正式环境：解决的问题如下：

    [x]edu3.0.7：大同小学：通讯录家长端只需要老师的联系方式，其他的不要（15116164882）
    [x]edu3.0.7：班级圈禁止评论功能
    [x]edu3.0.7：发起班级圈时可选择是否可评论
    [x]edu3.0.7：签到界面进入时弹提示（弹框至2020年1月14日）

11月08日教育加_3.0.6[apk]：正式环境：解决的问题如下：

    [x]edu3.0.6：OPPO R9s 拍照不能截图（改头像）

11月07日教育加_3.0.6[apk]：正式环境：解决的问题如下：

    [x]edu3.0.6：不能拍照：我的-个人头像修改
    [x]edu3.0.6：应用程序提示小红点
    [x]edu3.0.6：大同课表显示居中

10月09日教育加_3.0.5[apk]：正式环境：解决的问题如下：

    [x]edu3.0.5：班级圈照片预览不了
    [x]edu3.0.5：不能拍照：综合素质私信 学校通知 班级圈 失物招领 校长信箱

10月08日教育加_3.0.5(apk)：正式环境：解决的问题如下：

    [x]edu3.0.5：家庭作业不能拍照
    [x]edu3.0.5：家庭作业预览
    [x]edu3.0.5：家庭作业默认今天

9月20日教育加_3.0.4(apk)：正式环境：解决的问题如下：

    [x]edu3.0.4：艾道小程序恢复token方式登录
    [x]edu3.0.4：校长信箱可回复匿名信（app账号13908466951）
    [x]edu3.0.4：芙蓉区大同小学不显示“我的订单”（15973128808）
    [x]edu3.0.4：大同小学 取消年假 调休 增加 其他选项
    [x]edu3.0.4：教师请假界面：“请输入请假时长”改为“请假时长”
    [x]edu3.0.4：芙蓉区大同小学不显示“收货地址”（15973128808）
    [x]edu3.0.4：加载家庭作业列表的时候 需要能点击
    [x]edu3.0.4：大同小学教师通讯录改成群组模式（15973128808）

9月16日教育加_3.0.4(apk)：正式环境：解决的问题如下：

    [x]edu3.0.4：艾道小程序恢复token方式登录

9月9日教育加_3.0.3(apk)：正式环境：解决的问题如下：

    [x]edu3.0.3：更改艾道小程序跳转路径
    [x]edu3.0.3：bug：199号段电话号码不是有效的格式（文龙改）
    [x]edu3.0.3：bug：平安校园：视频界面，右上角 下一个 按钮右边有一条明显的黑色竖立的黑线
    [x]edu3.0.3：请刘振注意：在后台更改vip状态，在客户端能否使用下拉刷新更新vip图标
    [x]edu3.0.3：bug：去除老师通讯录界面下拉刷新黑边
    [x]edu3.0.3：bug：去除学生通讯录界面下拉刷新黑边

7月16日教育加_3.0.2(apk)：正式环境：解决的问题如下：

    [x]edu3.0.2：bug：班级圈删掉后不刷新
    [x]edu3.0.2：bug：老师进入失物招领界面显示空白，需要下拉刷新一下才会显示
    []edu3.0.2：bug（文龙改）：无法根据日期查询。如选择2019/5月份的，还是显示的全部的充值记录

7月11日教育加_3.0.2(apk)：正式环境：解决的问题如下：

    [x]edu3.0.1：bug：智慧校园的坨坨有时候刷不出来
    [x]edu3.0.2：发现界面的改版
    [x]edu3.0.2：宁远明德-学校多商户充值

7月1日教育加_3.0.1(apk)：正式环境：解决的问题如下：

    [x]edu3.0.1：bug：智慧校园的坨坨有时候刷不出来

6月28日教育加_3.0.1(apk)：正式环境：解决的问题如下：

    [x]edu3.0.1：优化vip信息的刷新机制（第一次进入主页，主动拉一次vip信息）
    [x]edu3.0.1：bug：失物招领，图片出错（小米6手机）
    [x]edu3.0.1：bug：学校通知，最多可选9张图（目前3张）
    [x]edu3.0.1：bug：校长信箱提示"content不能为空"
    [x]edu3.0.1：学校通知：bug：不填写标题提示title不能为空，不填写内容提示content为空

6月14日教育加_3.0.1(apk)：正式环境：解决的问题如下：

    [x]edu3.0.1：VIP新的续费界面
    [x]edu3.0.1：bug：修复推送功能（升级了版本）
    [x]edu3.0.1：我的服务：跳转vip充值界面
    [x]edu3.0.1：BaseFragment添加commonRefresh方法
    [x]edu3.0.1：bug(国志)：充值后刷新vip图标
    [x]edu3.0.1：bug(文龙)：四川教师通知发布不了

5月28日教育加_3.0.0(apk)：正式环境：解决的问题如下：

    [x]edu3.0.0：bug：家庭作业-作业完成标记：家庭作业没有 完成标记了。那个旗帜标记
    [x]edu3.0.0：bug：vip支付方式：首页头像点击支付，有三种方式支付。智慧校园只有微信
    [x]edu3.0.0：bug：综合素质-校内：家长评论校内（一条叫哦哦）的校内，评论一条出现两条一样的评论
    [x]edu3.0.0：bug：班级圈：同一个人在班级圈中发布两条信息，删除下面一条信息。会导致界面删除的那条上移，移动后覆盖没有删除的。刷新后正常
    [x]edu3.0.0：bug：学生请假：老师在修改学生请假的症状（就是那个标签）如果在修改时，直接点提交，修改的症状不会提交上，需要先保存后提交（是这样设计的，提交并不提交内容，保存才提交内容）
    [x]edu3.0.0：bug：学生请假：那个症状标签间隔较大
